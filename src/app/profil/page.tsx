...
export default function ProfilPage() {
  ...
  return (
    <div className="pt-0 bg-white animate-in fade-in duration-500">
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row gap-10 md:gap-20 items-center">
            <div className="w-full md:w-1/2 space-y-6 md:space-y-8">
              <div className="text-secondary font-bold tracking-widest uppercase text-[10px] px-4 py-2 bg-secondary/10 rounded-full inline-flex items-center gap-2">
      ...
      <section id="visi-misi" className="py-16 md:py-24 bg-slate-50">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12">
            <div className="bg-white p-8 md:p-16 rounded-xl shadow-xl border border-slate-100 space-y-6 md:space-y-8">
              <div className="bg-primary/10 w-14 h-14 md:w-20 md:h-20 rounded-xl flex items-center justify-center">
            ...
            <div className="bg-primary p-8 md:p-16 rounded-xl shadow-2xl text-white space-y-6 md:space-y-8 relative overflow-hidden">
              <div className="bg-white/10 w-14 h-14 md:w-20 md:h-20 rounded-xl flex items-center justify-center">
              ...
              <ul className="space-y-4 md:space-y-5">
                {settings?.mission?.length > 0 ? settings.mission.map((item: string, idx: number) => (
                  ...
                  <li key={i} className="flex gap-4 items-start group">
                    <CheckCircle2 className="h-5 w-5 md:h-7 md:w-7 text-secondary shrink-0 mt-1" />
                    <span className="text-white/90 text-sm md:text-lg font-medium leading-snug">{m}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section id="fasilitas" className="py-16 md:py-24">
        <div className="container mx-auto px-4">
           <div className="text-center mb-12 md:mb-20 space-y-4 md:space-y-6">
            <div className="text-secondary font-bold tracking-widest uppercase text-[10px] px-4 py-2 bg-secondary/10 rounded-full inline-flex items-center gap-2">
              <Building2 className="h-4 w-4" /> Sarana Prasarana
            </div>
            <h2 className="text-2xl md:text-5xl font-bold text-primary font-headline tracking-tighter">Fasilitas Unggulan Kami</h2>
            <p className="text-slate-400 text-sm md:text-base max-w-xl mx-auto">Infrastruktur modern yang menunjang kreativitas dan produktivitas siswa.</p>
          </div>
            ...
            {publishedFacilities.length > 0 ? publishedFacilities.map((f: any) => (
              <Card key={f.id} className="overflow-hidden border shadow-sm hover:shadow-md transition-all duration-500 rounded-xl group bg-white">
                ...
                <CardContent className="p-6 md:p-8">
                  <h4 className="text-xl md:text-2xl font-bold text-primary font-headline mb-3 group-hover:text-secondary transition-colors">{f.name || "Nama Fasilitas"}</h4>
        ...
        <section id="lokasi" className="py-16 md:py-24 bg-slate-50">
          <div className="container mx-auto px-4">
            <div className="text-center mb-12 space-y-4">
              ...
              <h2 className="text-2xl md:text-5xl font-bold text-primary font-headline tracking-tighter">Temukan Kami</h2>
              <p className="text-slate-500 text-sm md:text-base max-w-xl mx-auto font-medium">{settings.address}</p>
            </div>
            <div className="w-full h-[300px] md:h-[500px] rounded-xl overflow-hidden shadow-2xl border-[8px] md:border-[12px] border-white bg-white">
              <iframe 
